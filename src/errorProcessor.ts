...
type ErrorSeverity = "critical" | "warning" | "normal";

export class ErrorProcessor {
  static humanizeError(
    error: Error,
  ...
  static truncateMessage(message: string, maxLength: number = 150): string {
  ...
  static shouldIgnoreError(error: Error): boolean {
    const message = error.message || error.toString();
