export interface ErrorNarratorFilters {
  ignorePatterns?: string[];
  onlyPatterns?: string[] | null;
  errorTypes?: string[];
}

export interface ErrorNarratorConfig {
  enabled?: boolean;
  voice?: string | null;
  rate?: number;
  pitch?: number;
  ...
  cooldownMs?: number;
  filters?: ErrorNarratorFilters;
  humanize?: boolean;
  fallbackToRaw?: boolean;
  debug?: boolean;
}

const defaultConfig: Required<ErrorNarratorConfig> = {
  enabled: true,
  voice: null,
  rate: 1,
  pitch: 1,
...
export class Config {
  private config: Required<ErrorNarratorConfig>;
  private lastSpoken: Map<string, number>;
  private errorCounts: Map<string, number>;

  constructor(userConfig: ErrorNarratorConfig = {}) {
  ...
  private deepMerge(
    target: Required<ErrorNarratorConfig>,
    source: ErrorNarratorConfig
  ): Required<ErrorNarratorConfig> {
    const result = { ...target };

    for (const key in source) {
      const sourceValue = source[key as keyof ErrorNarratorConfig];
      ...
      if (
        ...
        (result[key as keyof Required<ErrorNarratorConfig>] as any) =
          this.deepMerge(
            result[key as keyof Required<ErrorNarratorConfig>] as any,
            sourceValue as any
          ) as any;
  ...
  shouldSpeak(error: Error): boolean {
    ...
    // Use the actual message content for the key instead of substring
    ...
    const errorTypes = this.config.filters.errorTypes;
    if (
      errorTypes &&
      errorTypes.length > 0 &&
  ...
  getConfig(): Required<ErrorNarratorConfig> {
    return { ...this.config };
  }
